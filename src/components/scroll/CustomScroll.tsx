...
function CustomScroll({
    ...
    hostClassName = '',
    viewportClassName = '',
    bottomReserveSelector,
}: {
    children: React.ReactNode;
    hostClassName?: string;
    viewportClassName?: string;
    bottomReserveSelector?: string;
}) {
    ...
    useEffect(() => {
        ...
        const syncTrack = () => {
            if (bottomReserveSelector) {
                const box = document.querySelector(bottomReserveSelector) as HTMLElement | null;
                const gap = 12;
    ...
    }, [syncThumb, bottomReserveSelector]);
    ...
    return (
        <div
            ref={hostRef}
            className={`customScroll-host${hostClassName ? ` ${hostClassName}` : ''}${visible && thumb.height > 0 ? ' scroll-visible' : ''}`}
        >
            <div
                ref={viewportRef}
                className={`customScroll-viewport${viewportClassName ? ` ${viewportClassName}` : ''}`}
            >
                {children}
            </div>
            {thumb.height > 0 && (
                <div ref={barRef} className="customScroll-bar" onMouseDown={onTrackClick}>
                    <div
                        ref={thumbRef}
                        className="customScroll-thumb"
